...
use Modules\Academic\Entities\ExamGroupClassBatchExamStudent;
...
class ExamScheduleController extends \Modules\Core\Http\Controllers\Api\Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $examSchedule = ExamGroupClassBatchExamStudent::where('student_session_id', $studentSession->id)
            ->whereHas('examGroupClassBatchExam', function ($query) {
                $query->where('is_active', 1);
            })
            ->with('examGroupClassBatchExam')
            ->get();
